...
async def periodic_update(request: Request):
    # This route seems to be duplicated with the HTML response route for "/"
    # Consider removing one of them depending on intended use.
    return JSONResponse(content={"message": "Hello World!"})

@router.get("/similars", tags=["rag"])
async def get_similars_for_tread(request: Request, id: str, auth: dict = Depends(get_api_key)):
    ...
    if conn:
        ...
        if similar_docs:
            similar_docs = get_all_data_similar_documents(similar_docs, os.getenv("MONGO_URL"), "postgres", conn)
            conn.close()
...
@router.post("/thread_sentiment", tags=["Analyse de sentiments"])
